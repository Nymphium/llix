...
spaces = function(head, ...)
  ...
  return not args[1] and head or head * V('Space') * spaces(unpack(args))
end
local keywords
keywords = function(head, ...)
  ...
  return not args[1] and K(head) or K(head) + keywords(unpack(args))
end
local lbl_tbl
...
gen_unoptbl = function(...)
  ...
  insert(args, t)
  return #args < 2 and (t.op and t or val) or gen_unoptbl(unpack(args))
end
local gen_exp
...
local llix = P({
  ...
  Value = CK('nil') + CK('false') + CK('true') + CV('Number') + V('String') + CP('...') + V('Funcdef') + V('Tableconstructor') + V('Funcall') + V('Var') + spaces(P('('), V('Exp'), P(')')),
  Exp = V('lor'),
